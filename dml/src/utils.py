...
def prepare_dml_data(
    ...
    Excludes outcome column AND underlying life_expectancy column to prevent data leakage.
    """
    covariate_cols = covariate_cols or get_feature_columns()
    # Exclude both the synthetic outcome AND the underlying life_expectancy column
    covariate_cols = [c for c in covariate_cols if c not in (outcome_col, "life_expectancy")]
